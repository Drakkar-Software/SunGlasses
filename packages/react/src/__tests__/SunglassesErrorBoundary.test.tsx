...
describe('SunglassesErrorBoundary (react)', () => {
  ...
  it('uses an explicit client prop over context', () => {
    const contextClient = makeClient();
    const propClient = makeClient();
    const container = document.createElement('div');
    document.body.appendChild(container);

    act(() => {
      createRoot(container).render(
        <SunglassesProvider client={contextClient}>
          <SunglassesErrorBoundary client={propClient}>
            <Bomb shouldThrow={true} />
          </SunglassesErrorBoundary>
        </SunglassesProvider>
      );
    ...
    expect(propClient.capture).toHaveBeenCalled();
